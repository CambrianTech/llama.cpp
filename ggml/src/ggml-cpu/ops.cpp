...
// ggml_compute_forward_situ (Kimi K3)
...
static void ggml_compute_forward_situ_f32(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; k++) {
            const float g = src0_p[k]; // gate
            float       u = src1_p[k]; // up
            // SiTU gate: beta * tanh(g/beta) * sigmoid(g)
            const float situ_g = beta * tanhf(g / beta) * (1.0f / (1.0f + expf(-g)));
            // optional up-transform; skipped when linear_beta <= 0
            if (linear_beta > 0.0f) {
