    ...
    if (op->op == GGML_OP_MUL_MAT_ID && op->src[3] != NULL) {
        ...
        if (gsrc1->type == GGML_TYPE_F32 && op->type == GGML_TYPE_F32) {
            ...
            if (op->ne[2] <= MMVQ_MAX_BATCH_SIZE && ggml_is_quantized(gsrc0->type)) {
                // ncols_dst>1 with ids routes to mul_mat_vec_q_MOE (mmvq.cu), ncols_dst==1 to
                // mul_mat_vec_q — BOTH are now gather-capable, so the whole mmvq family qualifies.
                // (The gate must track the kernel that ACTUALLY runs: that predicate is exactly
                // ne2 <= get_mmvq_mmid_max_batch, the dispatcher's own mmvq claim.)
